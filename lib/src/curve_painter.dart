...
class _CurvePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final trackGradientRotationAngle = appearance.counterClockwise ? startAngle + 10.0 : startAngle - 10.0;

    final GradientRotation trackRotation = GradientRotation(degreeToRadians(trackGradientRotationAngle));
    Paint trackPaint;
    ...
    final gradientRotationAngle = appearance.counterClockwise ? startAngle + 10.0 : startAngle - 10.0;
    ...
    final colors = dynamicGradient && appearance.counterClockwise ? appearance.progressBarColors.reversed.toList() : appearance.progressBarColors;
  ...
  drawCircularArc({required Canvas canvas, required Size size, required Paint paint, bool ignoreAngle = false, bool spinnerMode = false}) {
    ...
    canvas.drawArc(Rect.fromCircle(center: center!, radius: radius), degreeToRadians(spinnerMode ? 0 : startAngle), degreeToRadians(spinnerMode ? 360 : range + currentAngle), false, paint);
  }

  drawShadow({required Canvas canvas, required Size size}) {
    final shadowStep = appearance.shadowStep != null ? appearance.shadowStep! : math.max(1, (appearance.shadowWidth - appearance.progressBarWidth) ~/ 10);
