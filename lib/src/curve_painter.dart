...
class _CurvePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final gradientRotationAngle =
        dynamicGradient
            ? appearance.counterClockwise
                ? startAngle + 10.0
                : startAngle - 10.0
            : 0.0;
    final GradientRotation rotation = GradientRotation(
      degreeToRadians(gradientRotationAngle),
    );
  ...
  void drawShadow({required Canvas canvas, required Size size}) {
    ...
    for (int i = 1; i <= repetitions; i++) {
      ...
      shadowPaint.color = appearance.shadowColor.withValues(
        alpha: maxOpacity - (opacityStep * (i - 1)),
      );
